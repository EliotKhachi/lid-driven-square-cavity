...
beta = dx/dy;
alpha = 1/(2*(1+beta^2));
w_PSOR = 1.3; % relaxation parameter for PSOR
...
while t < t_f%current_error > ERROR_TOL % loop until error is within the tolerance
    ...
    for i = 2:N_y-1
        for j = 2:N_x-1
            u(i,j,n+1) = -(psi(i+1,j,n)-psi(i-1,j,n))/(2*dy);
            v(i,j,n+1) = -(psi(i,j+1,n)-psi(i,j-1,n))/(2*dx);
        end
    end
...
quiver(u_flipped(:,:), v_flipped(:,:), 1)
title(['Vorticity, Stream, and Velocity Fields for Re = ', Re_str, ' and N = ', N_str, 'x', N_str]);
%saveas(figure(3*(l-1)+r), ['Figure', num2str(3*(l-1)+r)], 'png')
end
end